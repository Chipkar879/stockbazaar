...
export default function SuperAdminDashboard() {
  ...
  const fetchMasterData = async () => {
    ...
    // Sort profiles by wallet balance to compute Global Rank
    ...
    // Fetch teachers awaiting verification letters audit
  ...
  const handleToggleFreezeStatus = async (profileId, currentFrozenState, profileEmail) => {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        alert("⚠️ Warning: Supabase RLS prevented row update. Please execute the SQL policy script in Supabase.");
        return;
      }

      const updatedProfile = data[0];
      const updatedTrial = calculateTrialStatus(updatedProfile);

      // Update active modal immediately
      ...
      // Re-fetch all data to keep background cards in sync
      await fetchMasterData();
  ...
  // Permanent Dossier Purge Handler
  const handleDeleteDossier = async (profileId, profileName) => {
    ...
    const confirmSecond = confirm(`🚨 FINAL WARNING: Deleting this dossier is permanent. Confirm deletion?`);
  ...
  // Dynamically Extract All Registered School IDs
