...
// Floating background items pushed far back into negative Z-space
const RainingAsset = ({ icon, isMobileHidden }) => {
  const [style, setStyle] = useState({});

  useEffect(() => {
    const depth = Math.floor(Math.random() * -150) - 50; // Pushed back (-50px to -200px)
    setStyle({
  ...
  return (
    <div
      className={`absolute top-[-5vh] text-[#7a0000]/25 pointer-events-none select-none animate-rainDown ${
...
export default function Home() {
  ...
  // Interactive 3D Camera Tilt Control State
  const containerRef = useRef(null);
  const [tilt, setTilt] = useState({ rotateX: 0, rotateY: 0 });

  // Handle subtle 3D camera movement based on mouse position
  const handleMouseMove = (e) => {
    ...
    setTilt({
      rotateX: -y * 14, // Pitch angle
      rotateY: x * 14,  // Yaw angle
    });
  };

  const handleMouseLeave = () => {
    setTilt({ rotateX: 0, rotateY: 0 }); // Reset camera position
  };
  ...
  return (
    <main 
      ...
      className="min-h-screen bg-black text-slate-100 antialiased font-sans relative max-w-full overflow-x-hidden pb-20 [perspective:1200px]"
    >
      <Navbar />

      {/* 3D THEATER SCENE CANVAS WRAPPER */}
      <div 
        ref={containerRef}
        className="w-full transition-transform duration-200 ease-out [transform-style:preserve-3d]"
        style={{
          transform: `rotateX(${tilt.rotateX}deg) rotateY(${tilt.rotateY}deg)`
        }}
      >
        {/* LAYER -1: DEEP BACKGROUND (Pushed far back -180px) */}
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden max-w-full [transform:translateZ(-180px)_scale(1.3)]">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-28 sm:pt-36 text-center space-y-8 relative z-10 [transform-style:preserve-3d]">
          
          {/* Ambient Glow Orb */}
          <div className="absolute top-10 left-1/2 -translate-x-1/2 w-[300px] sm:w-[600px] h-[200px] sm:h-[300px] bg-[#7a0000]/20 blur-[120px] rounded-full -z-10 [transform:translateZ(-60px)]" />

          {/* 3D FOREGROUND HEADLINE BLOCK */}
          <div className="space-y-4 max-w-3xl mx-auto select-none [transform-style:preserve-3d]">
            
            {/* 3D FOREGROUND BADGE (+50px) */}
            <div className="[transform:translateZ(50px)]">
              <span className="inline-flex items-center gap-2 bg-[#1a0808] border border-[#2b0808] text-[#ff3333] text-[11px] uppercase tracking-widest font-black px-4 py-1.5 rounded-full shadow-[0_10px_25px_rgba(0,0,0,0.8)] border-b-2 border-b-[#7a0000]">
                🚀 The Ultimate Classroom Sandbox Arena
              </span>
            </div>
            
            {/* 3D FOREGROUND HEADLINE (+80px) */}
            <h1 className="text-4xl sm:text-7xl font-black tracking-tight leading-none text-white [transform:translateZ(80px)] filter drop-shadow-[2px_4px_10px_rgba(0,0,0,0.9)]">
              <span className="inline-block [transform:translateZ(70px)] drop-shadow-[2px_2px_0px_#2b0808]">Own the Market</span> <br className="hidden sm:inline" />
              <span className="inline-block [transform:translateZ(70px)] drop-shadow-[2px_2px_0px_#2b0808]">Before the</span>{' '}
              
              {/* 3D "Bell Rings" (+110px Pop Out) */}
              <span className="inline-block bg-gradient-to-r from-[#e24040] via-[#bd7f7f] to-[#e24040] bg-clip-text text-transparent font-black [transform:translateZ(110px)] filter drop-shadow-[2px_2px_0px_#4a0000] drop-shadow-[4px_4px_0px_#1a0000] drop-shadow-[0_15px_30px_rgba(0,0,0,0.95)] hover:scale-105 transition-transform duration-300">
                Bell Rings
              </span>
            </h1>

            {/* 3D FOREGROUND SUBTITLE (+40px) */}
            <p className="text-slate-300 text-xs sm:text-base max-w-xl mx-auto leading-relaxed pt-3 font-medium [transform:translateZ(40px)] filter drop-shadow-[0_5px_15px_rgba(0,0,0,0.9)]">
          ...
          {/* 3D FOREGROUND USER DASHBOARD / ACTION CARD (+60px) */}
          <div className="flex flex-wrap justify-center items-center gap-4 pt-4 [transform:translateZ(60px)]">
            {loading ? (
              <div className="px-10 py-4 bg-[#1a0808] border border-[#2b0808] text-slate-500 rounded-2xl text-sm font-bold animate-pulse shadow-2xl">
                Syncing Terminal...
              </div>
            ) : !user ? (
              <div className="flex flex-wrap justify-center gap-4 [transform-style:preserve-3d]">
                <Link
                  href="/signup"
                  className="bg-[#7a0000] text-white font-poppins font-black px-10 py-4 rounded-2xl text-sm shadow-[0_15px_30px_rgba(122,0,0,0.4)] hover:bg-[#a30000] hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group border-b-4 border-b-[#4a0000] [transform:translateZ(70px)]"
                >
                ...
                <Link
                  ...
                  className="bg-[#0f0505] border-2 border-[#2b0808] text-slate-300 font-poppins font-black px-10 py-4 rounded-2xl text-sm hover:border-[#7a0000] hover:text-white transition-all shadow-[0_10px_25px_rgba(0,0,0,0.8)] [transform:translateZ(60px)]"
                >
                  Access Dashboard
                </Link>
              </div>
            ) : (
              <div className="w-full max-w-md bg-[#0f0505] border-2 border-[#2b0808] rounded-3xl p-6 text-left shadow-[0_20px_50px_rgba(0,0,0,0.9)] space-y-4 [transform:translateZ(65px)] border-b-4 border-b-[#2b0808]">
                
                {profile?.role === 'student' && !profile?.student_approved && (
                  <div className="bg-amber-950/40 border border-amber-900/60 text-amber-300 rounded-xl p-4 text-xs space-y-1 shadow-inner">
                ...
                <div className="flex items-center justify-between border-b border-[#2b0808] pb-3">
                  ...
                  <span className="px-2.5 py-0.5 bg-[#1a0808] border border-[#2b0808] rounded-full text-[9px] font-black uppercase tracking-wider text-[#ff3333] shadow-md">
                    {profile?.role || 'Personal'} Track
                  </span>
                </div>
                
                {((profile?.role === 'student' && profile?.student_approved) || profile?.role === 'personal') && (
                  <div className="bg-gradient-to-br from-[#7a0000] to-[#4a0000] p-4 rounded-xl text-white shadow-[0_10px_20px_rgba(122,0,0,0.3)] space-y-1 border border-[#a30000]/40 border-b-2 border-b-[#4a0000] [transform:translateZ(20px)]">
                    <p className="text-[9px] font-black uppercase tracking-wider text-rose-200/80">Available Sandbox Balance</p>
                    <p className="text-2xl font-poppins font-black tracking-tight font-mono">
        ...
        {/* 3D FOREGROUND TEACHER APPROVAL QUEUE (+50px) */}
        {profile?.role === 'teacher' && profile?.verification_status === 'approved' && pendingClassStudents.length > 0 && (
          <section className="max-w-md mx-auto px-4 sm:px-6 pt-6 relative z-20 animate-fadeInFast [transform:translateZ(50px)]">
            <div className="bg-[#0f0505] border-2 border-[#2b0808] rounded-3xl p-5 shadow-[0_20px_40px_rgba(0,0,0,0.9)] space-y-3">
        ...
        {/* 3D FOREGROUND LIVE MARKET SIMULATOR TERMINAL (+50px) */}
        <section id="simulator" className="max-w-7xl mx-auto px-4 sm:px-6 pt-28 relative z-10 flex flex-col items-center justify-center animate-fadeIn scroll-mt-24 [transform:translateZ(50px)]">
          ...
          <div className="bg-[#0f0505] border-2 border-[#2b0808] rounded-3xl shadow-[0_25px_60px_rgba(0,0,0,0.95)] w-full max-w-4xl p-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-center border-b-4 border-b-[#2b0808]">
            <div className="col-span-1 space-y-3">
              <div className="text-[11px] font-black uppercase text-slate-500 tracking-wider px-1">Select Live Stream Desk:</div>
              <button
                onClick={() => setActiveTab('stocks')}
                className={`w-full flex justify-between items-center px-4 py-3.5 rounded-2xl border text-left text-xs uppercase tracking-wider font-black transition-all ${activeTab === 'stocks' ? 'bg-[#7a0000]/20 border-[#7a0000] text-[#ff3333] shadow-md' : 'bg-[#1a0808] border-[#2b0808] text-slate-400 hover:bg-[#2b0808]'}`}
              >
              ...
              <button
                ...
                className={`w-full flex justify-between items-center px-4 py-3.5 rounded-2xl border text-left text-xs uppercase tracking-wider font-black transition-all ${activeTab === 'crypto' ? 'bg-[#7a0000]/20 border-[#7a0000] text-[#ff3333] shadow-md' : 'bg-[#1a0808] border-[#2b0808] text-slate-400 hover:bg-[#2b0808]'}`}
              >
        ...
        {/* 3D FOREGROUND INTEGRATED LEARNING ACADEMY CARDS (+40px) */}
        <section id="courses" className="max-w-6xl mx-auto px-4 sm:px-6 pt-32 relative z-10 space-y-8 scroll-mt-24 [transform:translateZ(40px)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 [transform-style:preserve-3d]">
            <div className="bg-[#0f0505] border-2 border-[#2b0808] p-6 rounded-3xl space-y-4 shadow-[0_15px_35px_rgba(0,0,0,0.8)] [transform:translateZ(30px)] hover:[transform:translateZ(50px)] transition-transform duration-300">
        ...
        {/* 3D FOREGROUND CAMPUS DEPLOYMENT BANNER (+50px) */}
        <section id="pricing" className="max-w-7xl mx-auto px-4 sm:px-6 pt-32 relative z-10 scroll-mt-24 [transform:translateZ(50px)]">
          <div className="bg-gradient-to-br from-[#0f0505] to-[#1a0808] border-2 border-[#2b0808] text-white rounded-3xl p-8 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.9)] relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-8 border-b-4 border-b-[#7a0000]">
            ...
            <div className="w-full md:w-auto z-10 flex flex-col sm:flex-row items-center gap-4 justify-center">
              <Link
                href="/signup"
                className="px-6 py-4 bg-[#7a0000] hover:bg-[#a30000] text-white text-xs font-black uppercase tracking-wider rounded-xl shadow-lg border-b-2 border-b-[#4a0000] transform hover:-translate-y-0.5 transition-all text-center w-full sm:w-auto whitespace-nowrap"
              >
