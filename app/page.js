...
export default function Home() {
  ...
  // Institutional school & class architecture states
  ...
  // Complete Original Mock Data Arrays (Restored to maintain project scale)
  const MOCK_STOCKS = [
  ...
  const fetchInstitutionalEcosystem = async (currentProfile) => {
    ...
    // 1. Fetch classrooms built inside this school hub code
    ...
    // 3. Sub loop: Pull pending students whose class container is assigned to this logged-in teacher
  ...
  return (
    <main className="min-h-screen bg-[#f5f7ff] text-[#1e1b4b] antialiased pb-20 relative overflow-x-hidden">
        ...
        <div className="flex justify-center pt-4">
          {loading ? (
            ...
            <div className="w-full max-w-2xl bg-white border border-slate-200 rounded-3xl p-6 text-left shadow-md space-y-4">
              ...
              {/* PERSONAL TRADER DASHBOARD VIEW */}
              {profile?.role === 'personal' && (
                <div className="space-y-4">
                  ...
                  {/* Personal Track Account Cleanup Danger Zone */}
                  <div className="p-4 border border-rose-100 bg-rose-50/20 rounded-2xl space-y-2">
                    <h4 className="text-xs font-black text-rose-600 uppercase tracking-wider">⚠️ Account Danger Zone</h4>
                    <p className="text-[11px] text-slate-500">Wiping your account instantly deletes your profile rows and resets trading transaction data history.</p>
                    <button onClick={handleDeletePersonalAccount} className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white font-bold text-[11px] rounded-xl transition-all">
                      Delete My Account & Wipe Wallet Data
                    </button>
                  </div>
                </div>
              )}

              {/* PRIYANKA MISS & COORDINATOR DELEGATION BOARD */}
              {profile?.role === 'teacher' && profile?.verification_status === 'approved' && (
                <div className="space-y-6">
                  ...
                  {/* REAL-TIME STUDENT REQUESTS PANEL */}
      ...
      {/* COMPLETE MARKET WATCH INTERACTIVE MODULE */}
      <section className="max-w-4xl mx-auto px-6 pt-24 relative z-10 space-y-6">
      ...
      {/* INSTITUTIONAL CONTEXT FOOTER BLOCK (CLEANED UP - 'SB' TEXT REMOVED FULLY) */}
      <section className="max-w-7xl mx-auto px-6 pt-24 relative z-10">
