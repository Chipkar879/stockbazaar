...
export default function SuperAdminDashboard() {
  ...
  const fetchMasterData = async () => {
    ...
    // Calculate global rank for all profiles based on wallet_balance descending
  ...
  // Dynamic School Buttons Extracted from Profiles
        ...
        <div className="border-b border-[#2b0808] pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium mt-1">
              Click any dossier card below to inspect full records, portfolio rank, wallet standings, or execute node deletions.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-3.5 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all border ${selectedSchoolFilter === 'ALL' ? 'bg-[#7a0000] border-[#a30000] text-white shadow-md' : 'bg-[#1a0808] border-[#2b0808] text-slate-400 hover:text-white hover:border-[#7a0000]'}`}
              >
              ...
              <button
                ...
                className={`px-3.5 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all border ${selectedSchoolFilter === 'PERSONAL' ? 'bg-[#7a0000] border-[#a30000] text-white shadow-md' : 'bg-[#1a0808] border-[#2b0808] text-slate-400 hover:text-white hover:border-[#7a0000]'}`}
              >
              ...
              {schoolList.map(schoolId => {
                ...
                return (
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider transition-all border ${selectedSchoolFilter === schoolId ? 'bg-[#7a0000] border-[#a30000] text-white shadow-md' : 'bg-[#1a0808] border-[#2b0808] text-slate-400 hover:text-white hover:border-[#7a0000]'}`}
                  >
                    🏫 {schoolId} ({count})
                  </button>
                );
          ...
          <div className="bg-[#0f0505] border-2 border-[#2b0808] rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-scaleUp">
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-left flex-1">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-bold">
                ...
                {/* 3. SCHOOL HUB CODE (Greyed if personal track) */}
                <div className={`p-3.5 rounded-xl space-y-1 transition-all ${selectedProfile.role === 'personal' ? 'bg-[#0f0505] border border-[#2b0808]/50 opacity-40 select-none' : 'bg-[#1a0808] border border-[#2b0808]'}`}>
                  ...
                  <p className="text-white font-mono">
                    {selectedProfile.role === 'personal' ? 'N/A (Personal Track)' : (selectedProfile.school_id || 'Not Enrolled')}
                  </p>
                </div>

                {/* 4. SPECIFIC CLASS ID (Greyed if personal track) */}
                <div className={`p-3.5 rounded-xl space-y-1 transition-all ${selectedProfile.role === 'personal' ? 'bg-[#0f0505] border border-[#2b0808]/50 opacity-40 select-none' : 'bg-[#1a0808] border border-[#2b0808]'}`}>
                  <span className="text-[10px] uppercase text-slate-500 tracking-wider">Specific Class Container ID</span>
                  <p className="text-white font-mono">
                    {selectedProfile.role === 'personal' ? 'N/A (Personal Track)' : (selectedProfile.specific_class_id || 'Unassigned')}
                  </p>
                </div>

                {/* 5. QUIZ STANDINGS (Greyed if personal track) */}
                <div className={`p-3.5 rounded-xl space-y-1 transition-all ${selectedProfile.role === 'personal' ? 'bg-[#0f0505] border border-[#2b0808]/50 opacity-40 select-none' : 'bg-[#1a0808] border border-[#2b0808]'}`}>
                  <span className="text-[10px] uppercase text-slate-500 tracking-wider">Quiz Standings Points</span>
                  <p className="text-amber-400 font-mono">
                    {selectedProfile.role === 'personal' ? 'N/A (Personal Track)' : `${selectedProfile.quiz_points || 0} PTS`}
                  </p>
                </div>

                {/* 6. STUDENT VERIFICATION STATUS (Greyed if personal track) */}
                <div className={`p-3.5 rounded-xl space-y-1 transition-all ${selectedProfile.role === 'personal' ? 'bg-[#0f0505] border border-[#2b0808]/50 opacity-40 select-none' : 'bg-[#1a0808] border border-[#2b0808]'}`}>
                ...
                {/* 7. TEACHER AUDIT STATUS (Greyed if personal track) */}
                <div className={`p-3.5 rounded-xl space-y-1 transition-all ${selectedProfile.role === 'personal' ? 'bg-[#0f0505] border border-[#2b0808]/50 opacity-40 select-none' : 'bg-[#1a0808] border border-[#2b0808]'}`}>
